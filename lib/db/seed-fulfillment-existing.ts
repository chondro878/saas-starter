...
async function seedFulfillment() {
  ...
  console.log('📧 Which email do you want to use?');
  ...
  // Use the first owner account  
  const user = allUsers.find(u => u.role === 'owner') || allUsers[0];
  ...
  // Check if user already has a return address
  const existingAddresses = await db.select().from(userAddresses).where(eq(userAddresses.userId, user.id));
  
  let returnAddress;
  if (existingAddresses.length > 0) {
    returnAddress = existingAddresses.find(a => a.isDefault === 1) || existingAddresses[0];
    console.log('✅ Using existing return address');
  } else {
    // Add user's return address (default)
    [returnAddress] = await db.insert(userAddresses).values({
      userId: user.id,
      isDefault: 1,
